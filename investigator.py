import os
import zipfile
import mimetypes
from datetime import datetime
import streamlit as st
from PIL import Image
from PIL.ExifTags import TAGS, GPSTAGS
from geopy.geocoders import Nominatim

# Initialize geolocator (with a custom user_agent for API rules)
geolocator = Nominatim(user_agent="janus_adie_investigator")

def get_gps_info(exif_data):
    gps_info = {}
    if not exif_data:
        return None
    for tag, value in exif_data.items():
        tag_name = TAGS.get(tag, tag)
        if tag_name == "GPSInfo":
            for gps_tag in value:
                sub_tag_name = GPSTAGS.get(gps_tag, gps_tag)
                gps_info[sub_tag_name] = value[gps_tag]
    return gps_info

def convert_to_degrees(value):
    """Helper function to convert the GPS camera fractions to degrees."""
    d = float(value[0])
    m = float(value[1])
    s = float(value[2])
    return d + (m / 60.0) + (s / 3600.0)

def get_lat_lon(gps_info):
    if not gps_info or 'GPSLatitude' not in gps_info or 'GPSLongitude' not in gps_info:
        return None, None
        
    lat = convert_to_degrees(gps_info['GPSLatitude'])
    if gps_info.get('GPSLatitudeRef', 'N') != 'N':
        lat = -lat

    lon = convert_to_degrees(gps_info['GPSLongitude'])
    if gps_info.get('GPSLongitudeRef', 'E') != 'E':
        lon = -lon

    return lat, lon

# --- Inside your investigate_file function, update the image codepath to this: ---
# (Replace the old image IF condition with this code block)

    if mime_type and mime_type.startswith('image'):
        st.image(file_data, caption=file_name, use_container_width=True)
        
        try:
            img = Image.open(file_data)
            exif_data = img._getexif()
            
            # 1. Format Date and Time to DD/MM/YYYY HH:MM:SS
            raw_date = exif_data.get(36867) if exif_data else None
            formatted_date = "Unknown"
            if raw_date:
                try:
                    # EXIF standard format is usually "YYYY:MM:DD HH:MM:SS"
                    dt_obj = datetime.strptime(str(raw_date), "%Y:%m:%d %H:%M:%S")
                    formatted_date = dt_obj.strftime("%d/%m/%m %H:%M:%S")
                except ValueError:
                    formatted_date = str(raw_date)

            # 2. Extract and Reverse Geocode GPS Data
            gps_raw = get_gps_info(exif_data) if exif_data else None
            location_display = "Not Found"
            
            if gps_raw:
                lat, lon = get_lat_lon(gps_raw)
                if lat and lon:
                    try:
                        # Fetch the actual location name safely
                        location = geolocator.reverse((lat, lon), timeout=3)
                        location_name = location.address if location else "Unknown Location"
                    except Exception:
                        location_name = "Lookup Timeout/Error"
                    
                    location_display = f"{location_name}\n({lat:.6f}, {lon:.6f})"

            # Extract Camera Company and Model
            make = exif_data.get(271, "Unknown") if exif_data else "Unknown"
            model = exif_data.get(272, "Unknown") if exif_data else "Unknown"
            
            # Display Main Data cleanly
            col1, col2 = st.columns(2)
            with col1:
                st.metric(label="📸 Camera/Phone Company", value=str(make))
                st.metric(label="📱 Model", value=str(model))
            with col2:
                st.metric(label="📅 Date and Time", value=formatted_date)
                
            # Separate UI block for long location text
            st.markdown("### 📍 GPS Location")
            st.text(location_display)
            
            # Dump everything else into a hidden "More Data" section
            with st.expander("➕ More Data / Raw Metadata Dump"):
                if exif_data:
                    exif_dict = {}
                    for tag, value in exif_data.items():
                        tag_name = TAGS.get(tag, tag)
                        exif_dict[str(tag_name)] = str(value)
                    st.json(exif_dict)
                else:
                    st.write("No deep EXIF metadata available.")
                    
        except Exception as e:
            st.error(f"Error reading EXIF: {e}")
